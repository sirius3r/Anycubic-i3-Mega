          ...
          case '0':
            switch (SelectedDirectory[2]) {
              ...
              case '5': // "<05PrehtBed>"
                ...
                ExtUI::injectCommands_P(PSTR("M140 S60"));
                break;
              ...
              case '8': // "<08HtEndPID>"
                ...
                ExtUI::injectCommands_P(PSTR("M106 S190\nG4 P500\nM303 E0 S200 C10 U1\nM500\nM107\nM300 P105 S1661\nM300 P210 S1108"));
                break;

              case '9': // "<09HtBedPID>"
                SERIAL_ECHOLNPGM("Special Menu: Auto Tune Hotbed Pid");
                ExtUI::injectCommands_P(PSTR("M303 E-1 S60 C10 U1\nM500\nM300 P105 S1661\nM300 P210 S1108"));
                break;
...
void AnycubicTFTClass::RenderSpecialMenu(uint16_t selectedNumber) {
  switch (selectedNumber) {
      ...
      case 0: // First Page
        ...
        SENDLINE_PGM("<Exit>");
        SENDLINE_PGM("<01BLTL>");
        SENDLINE_PGM("<BL Touch Leveling>");
        SENDLINE_PGM("<02HtEndPID>");
        SENDLINE_PGM("<Auto Tune Hotend PID>");
        SENDLINE_PGM("<03HtBedPID>");
        SENDLINE_PGM("<Auto Tune Hotbed PID>");
        break;

      case 4: // Second Page
        SENDLINE_PGM("<04FWDeflts>");
        SENDLINE_PGM("<Load FW Defaults>");
        SENDLINE_PGM("<05PrehtBed>");
        SENDLINE_PGM("<Preheat Bed>");
        SENDLINE_PGM("<Exit>");
